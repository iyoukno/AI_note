...
import torch
import sys
sys.path.append(r'E:\project\yolov5_infer_linux\pac')
...
class Detector():
    ...
    def __init__(self, weights, device='cpu', classes=None, conf_thres=0.35, iou_thres=0.35,dnn=False, half=False,data=ROOT / "pac/config/coco128.yaml",):
        self.device = select_device(device) # cuda device, i.e. 0 or 0,1,2,3 or cpu
    ...
    def __call__(self,
        ...
        for path, im, im0s, vid_cap, s in dataset:
            ...
            # NMS
            with dt[2]:
                pred = non_max_suppression(pred, self.conf_thres, self.iou_thres, self.classes, agnostic_nms, max_det=max_det)
...
if __name__ == '__main__':
    model_path = ROOT / 'model_dir/yolov5s.onnx'
    device = '0'
    classes = [0,1]
    img = r'E:\project\yolov5_infer_linux\pac\detection\data\bus.jpg'
    # img = ROOT/'/pac/detection/data/bus.jpg'
    detector = Detector(weights=model_path, device=device, classes=classes)
    res = detector(source=img)
    print(res)